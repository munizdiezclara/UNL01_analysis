...
In stage 2, the C-C group continued with a high accuracy, and group U-U, kept increasing in accuracy. However, group C-U showed a decrease in the accuracy, to levels similar to the U-U group. A mixed model ANOVA did not find a significant effect of the *block* (`r apa(ANOVA_resp_s2, effect = "block" )`, `r report_BF_and_error(bay_ANOVA_resp_s2[1])`), but both the main effect of the *group* and the*groupXblock* interaction were significant(Group: `r apa(ANOVA_resp_s2, effect = "condition")`, `r report_BF_and_error(bay_ANOVA_resp_s2[2])`; Interaction: `r apa(ANOVA_resp_s2, effect = "condition:block")`, `r report_BF_and_error(bay_ANOVA_resp_s2_int[1])`). Simple main effects showed an effect of the condition in all blocks (*F* (2, 22) > 7.301, *p* < .001). Simple main effects of condition showed that there were significant differences in block 7, but not in any more of them. Simple comparisons showed that, in block 7, group U-U was significantly different from group C-C (*t*(`r pw_ANOVA_s2[2, 5]`) = `r pw_ANOVA_s2[2, 6]`, *p* = `r pw_ANOVA_s2[2, 7]`) and group C-U (*t*(`r pw_ANOVA_s2[3, 5]`) = `r pw_ANOVA_s2[3, 6]`, *p* = `r pw_ANOVA_s2[3, 7]`). 
